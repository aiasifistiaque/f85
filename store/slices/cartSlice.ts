...
const calculateTotals = (state: State) => {
	state.subTotal = state.cartItems.reduce((total: number, cartItem: CartItem) => {
		// Use unitPrice instead of price for calculation
		return total + cartItem.unitPrice * cartItem.qty;
	}, 0);

	state.vat = state.cartItems.reduce((total: number, cartItem: CartItem) => {
		if (cartItem.vat) {
			return total + ((cartItem.unitPrice * cartItem.vat) / 100) * cartItem.qty;
		}
		return total;
	}, 0);
	...
	reducers: {
		...
		addToCart: (state, action) => {
			...
			if (existItem) {
				...
				// ✅ FIX: Just update qty, don't recalculate price here
				state.cartItems = state.cartItems.map((stateItem: CartItem) =>
					stateItem.uniqueId === uniqueId
						? {
								...stateItem,
								qty: updatedQty,
								// Keep unitPrice unchanged, price field is not used in calculations
						  }
						: stateItem
				);
			} else {
				const newItem: CartItem = {
					...
					id: item.id || item._id,
					name: item.name,
					price: item.price, // Store unit price, not total
		...
		deleteOneFromCart: (state, action) => {
			...
			if (findItem) {
				if (findItem.qty > 1) {
					// ✅ FIX: Just decrement qty, don't recalculate price
					state.cartItems = state.cartItems.map((stateItem: CartItem) =>
						stateItem.uniqueId === uniqueId
							? {
									...stateItem,
		...
		// ✅ BONUS: Add this new reducer for direct quantity updates
		updateCartItemQuantity: (state, action) => {
			const { uniqueId, qty } = action.payload;

			if (qty <= 0) {
				// Remove item if quantity is 0 or less
				state.cartItems = state.cartItems.filter((item: CartItem) => item.uniqueId !== uniqueId);
			} else {
				const item = state.cartItems.find((item: CartItem) => item.uniqueId === uniqueId);

				if (item) {
					// Check stock
					const availableStock = item.variantStock || Infinity;
					...
					state.cartItems = state.cartItems.map((stateItem: CartItem) =>
						stateItem.uniqueId === uniqueId ? { ...stateItem, qty } : stateItem
					);
				}
			}
...
export const {
	...
	removeAddress,
	setToggleCart,
	updateCartItemQuantity, // ✅ Export the new action
} = cartSlice.actions;
