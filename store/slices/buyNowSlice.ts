...
export const buyNowSlice = createSlice({
	...
	reducers: {
		...
		// ✅ NEW: Direct quantity update reducer
		updateBuyNowCartItemQuantity: (state, action) => {
			const { uniqueId, qty } = action.payload;

			if (qty <= 0) {
				// Remove item if quantity is 0 or less
				state.cartItems = state.cartItems.filter(
					(item: CartItem) => item.uniqueId !== uniqueId
				);
			} else {
...
export const {
	...
	removeNowAddress,
	setNowToggleCart,
	updateBuyNowCartItemQuantity, // ✅ Export the new action
} = buyNowSlice.actions;
